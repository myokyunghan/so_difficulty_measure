...
class Annotation_Experiment_MD:
    ...
    def run(self):
        ...
        self.logger.info(f'>>>>>>>>>>>>>>>! start write_prompt')
        message_list    = write_prompt_exp(self.golden_df, e_f_dict, self.few_shot_n, self.sys_prompt, self.tk)
        self.logger.info(f'>>>>>>>>>>>>>>>! end write_prompt : {len(message_list)}')
    ...
    def set_eval_df(self, r_df):
        self.eval_df = pd.merge(self.golden_df, r_df, on='id')
