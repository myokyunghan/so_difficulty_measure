...
class Annotation_Validation_MD:
    ...
    def __init__(self, user_option):  
        ...
        # param
        Validation_option   = user_option['experiment_option']
        self.annotation_file_path = user_option['annotation_file_path']
        self.validation_file_path = user_option['validation_file_path']
        
        self.llm_model   = Validation_option['llm_model']
        self.model_name  = Validation_option['model_ver']
        self.app_model   = None
        self.few_shot_n  = Validation_option['few_shot_n']
        self.test_n      = Validation_option['test_n']
        self.q_src_yn    = Validation_option['q_src_yn']
        self.sys_prompt  = prompt[Validation_option['prompt_ver']]
        self.p_ver       = Validation_option['prompt_ver']
        self.sc_num      = Validation_option['sc_num']
        self.temperature = Validation_option['temperature']
        self.excel_ver   = Validation_option['excel_ver']
        self.loop_i      = Validation_option['loop_num']
    ...
    def run(self):
        ...
        self.logger.info(f'>>>>>>>>>>>>>>>! start get_annotation_data')
        ...
        self.logger.info(f'>>>>>>>>>>>>>>>! start get_annotation_data')
        self.validation_df  = get_annotation_data(self.validation_file_path)
        self.logger.info(f'>>>>>>>>>>>>>>>! end get_annotation_data : {self.validation_df.shape}')
        
        self.logger.info(f'>>>>>>>>>>>>>>>! start select_eval_q')
        eval_q_id_list  = select_eval_q(self.validation_df, self.test_n)
        self.logger.info(f'>>>>>>>>>>>>>>>! end select_eval_q : {eval_q_id_list}')

        self.logger.info(f'>>>>>>>>>>>>>>>! start select_fewshot_for_e')
        e_f_dict        = select_fewshot_for_e(self.golden_df, eval_q_id_list, self.few_shot_n, self.sc_num, self.test_n)
        self.logger.info(f'>>>>>>>>>>>>>>>! end select_fewshot_for_e : {e_f_dict.keys()}')

        self.logger.info(f'>>>>>>>>>>>>>>>! start write_prompt_vali')
        message_list    = write_prompt_vali(self.golden_df, self.validation_df, e_f_dict, self.few_shot_n, self.sys_prompt, self.tk)
        self.logger.info(f'>>>>>>>>>>>>>>>! end write_prompt_vali : {len(message_list)}')

        self.logger.info(f'>>>>>>>>>>>>>>>! start calc_acc')
        r_df            = self.calc_acc(message_list)
        self.logger.info(f'>>>>>>>>>>>>>>>! end calc_acc : {r_df.shape}')
    ...
    def set_eval_df(self, r_df):
        self.eval_df = pd.merge(self.validation_df, r_df, on='id')
