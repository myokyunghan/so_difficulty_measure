...
class CognitiveComplexityCalculator:
    ...
    def calculate(self):
        ...
        for ch in self.tree.root_node.children:
            if ch.type in ("function_declaration", "function_definition_statement"):
                self._proc(ch)
        return self.results
    ...
    def _v(self, n, ne):
        ...
        if t in ("for_statement", "for_generic_statement"):
        ...
        if t == "repeat_statement":
            self._a(n, "repeat", 1, ne); c = 1 + ne
            body = n.child_by_field_name("body")
            if body: c += self._vc(body, ne+1)
            return c

        if t == "goto_statement":
            self._a(n, "goto", 1, 0); return 1

        if t == "binary_expression": return self._hbin(n, ne)

        if t == "function_definition":  # anonymous function
            body = n.child_by_field_name("body")
            return self._vc(body, ne+1) if body else 0
    ...
    def _hif(self, n, ne):
        ...
        # elseif and else are separate children
        for ch in n.children:
            if ch.type == "elseif_statement":
                c += 1; self._a(ch, "elseif", 1, 0)
                econd = ch.child_by_field_name("condition")
                if econd: c += self._v(econd, ne)
                econs = ch.child_by_field_name("consequence")
                if econs: c += self._vc(econs, ne+1)
            elif ch.type == "else_statement":
                c += 1; self._a(ch, "else", 1, 0)
                for sub in ch.children:
                    if sub.type == "block": c += self._vc(sub, ne+1)
        alt = n.child_by_field_name("alternative")
        if alt and alt.type == "else_statement" and alt not in [ch for ch in n.children if ch.type == "else_statement"]:
    ...
    def _hbin(self, n, ne):
        ...
        for op in ops:
            if prev is None or op != prev:
                c += 1; self._ar(f"logical sequence '{op}'" if prev is None else f"logical change to '{op}'", 1); prev = op
        return c
