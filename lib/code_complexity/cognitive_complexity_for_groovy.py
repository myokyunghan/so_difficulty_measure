...
class CognitiveComplexityCalculator:
    ...
    def _walk(self, node):
        for ch in node.children:
            if ch.type in ("function_definition","method_declaration"): self._proc(ch)
            elif ch.type in ("class_declaration"): self._wcls(ch)
    ...
    def _v(self, n, ne):
        ...
        if t=="closure":
            body=n.child_by_field_name("body")
            if body: return self._vc(body,ne+1)
    ...
    def _hif(self, n, ne, first):
        ...
        alt=n.child_by_field_name("alternative")
        if alt:
            if alt.type=="if_statement": c+=self._hif(alt,ne,False)
...
def calculate_directory(d):
    ...
    for root,_,files in os.walk(d):
        for f in sorted(files):
            if any(f.endswith(e) for e in (".groovy",".gradle")):
