...
class PlotGen:
    ...
    def set_title_and_labels(title, x_label, y_label):
        ...
        Returns:
            None
        """
        plt.title(title, fontsize=figure_setting.FONT['title'])
        if x_label is not None:
            plt.xlabel(x_label, fontsize=figure_setting.FONT['label'])
        if y_label is not None:
            plt.ylabel(y_label, fontsize=figure_setting.FONT['label'])
    ...
    def draw_regression(self, ax, title, series, panel_label=''):
        ...
        # 제목 — 폰트 크기 통일
        # ax.set_title(f'Changes in {title} (topic)', fontsize=figure_setting.FONT['title'], pad=22)
